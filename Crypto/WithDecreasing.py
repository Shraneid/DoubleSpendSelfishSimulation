...
class Selfish_Mining:
    ...
    def Simulate(self):
        ...
        # Simulating all times where blocks have been found since starting t=0
        ...
        for i in range(0, len(SepBlocksEach2016)):
            TimesBlocksFoundSM = map(lambda x: x + self.__currentTimestamp, list(
                np.cumsum(np.random.exponential(1 / self.__alpha * 10 / self.__B, SepBlocksEach2016[i]))))
            TimesBlocksFoundHM = map(lambda x: x + self.__currentTimestamp, list(
                np.cumsum(np.random.exponential(1 / (1 - self.__alpha) * 10 / self.__B, SepBlocksEach2016[i]))))
        ...
        if self.__Sn0 is not None:
            considering_time_stats = \
                'Sn0 : ' + str(int(self.__Sn0)) + ' minutes \n' \
                                                  ...
                                                  '=> will change to : ' + str(
                    round(20160 / self.__Sn0, 4) * 100) + '% of initial value \n'
